...
class ApplicationPredictionActions:
    ...
    def creer_widgets(self):
        ...
        # (UI) On garde le même champ mais on change juste le texte : unités => filtres CNN
        ttk.Label(params, text="Filtres CNN:").grid(row=0, column=0, sticky='w', pady=2)
        self.entree_unites = ttk.Entry(params, width=8)
        self.entree_unites.grid(row=0, column=1, pady=2, padx=5)
        self.entree_unites.insert(0, "64")
...
3. Paramètres du modèle:
   - Filtres CNN: Complexité du modèle (32-128 recommandé)
...
5. Visualisation:
   ...
   - La zone de texte affiche les métriques de performance"""
    ...
    # =========================
    # ✅ MODELE CNN (Conv1D) à la place de LSTM
    # =========================
    def construire_modele(self, filtres, nb_caracteristiques):
        modele = Sequential([
            Input(shape=(self.jours_retrospection, nb_caracteristiques)),

            Conv1D(filters=filtres, kernel_size=3, activation='relu', padding='causal'),
            MaxPooling1D(pool_size=2),

            Conv1D(filters=max(16, filtres // 2), kernel_size=3, activation='relu', padding='causal'),
            Dropout(0.2),
    ...
    def entrainer_et_predire(self):
        ...
        try:
            filtres = int(self.entree_unites.get())
            if filtres <= 0:
                raise ValueError
        except ValueError:
            messagebox.showerror("Erreur", "Valeur de filtres CNN invalide")
            self.journaliser("Erreur: Filtres CNN invalides", 'erreur')
            return
        ...
        try:
            ...
            self.model = self.construire_modele(filtres, nb_caracteristiques)
